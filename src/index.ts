        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (nodeError.code === "EACCES") {
                    throw new Error(
                        `Permission denied accessing key file: ${resolvedKeyPath}. Please check file permissions.`
                    );
                } else if (nodeError.code === "ENOENT") {
                    throw new Error(
                        `Key file not found: ${resolvedKeyPath}. Please verify the file path.`
                    );
                } else {
                    throw new Error(
                        `Unable to access key file: ${resolvedKeyPath}. Error: ${nodeError.message}`
                    );
                }
            } else {
                throw new Error(
                    `Unexpected error accessing key file: ${resolvedKeyPath}`
                );
            }
        }
...
function parseArgs(): ServerConfig {
    ...
    if (!config.projectId) {
        throw new Error(
            "Missing required argument: --project-id or GOOGLE_PROJECT_ID in .env"
        );
    }
...
try {
    ...
    console.error(
        `Initializing BigQuery with project ID: ${config.projectId} and location: ${config.location}`
    );
...
function qualifyTablePath(sql: string, projectId: string): string {
    ...
    const unqualifiedPattern =
        /FROM\s+(?:(\w+)\.)?INFORMATION_SCHEMA\.TABLES/gi;
    return sql.replace(unqualifiedPattern, (match, dataset) => {
        if (dataset) {
            return `FROM \`${projectId}.${dataset}.INFORMATION_SCHEMA.TABLES\``;
        }
        throw new Error(
            "Dataset must be specified when querying INFORMATION_SCHEMA (e.g. dataset.INFORMATION_SCHEMA.TABLES)"
        );
    });
}
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
    try {
        ...
        for (const dataset of datasets) {
            ...
            console.error(
                `Found ${tables.length} tables and views in dataset ${dataset.id}`
            );
            ...
            for (const table of tables) {
                ...
                const resourceType =
                    metadata.type === "VIEW" ? "view" : "table";

                resources.push({
                    uri: new URL(
                        `${dataset.id}/${table.id}/${SCHEMA_PATH}`,
                        resourceBaseUrl
                    ).href,
